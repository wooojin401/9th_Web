...
++ 추가적으로 **dispatch 함수와 클로저의 관계**

**클로저의 동작 과정**

1. 컴포넌트가 렌더링 될 때 `useReducer(reducer, initialState)` 호출
2. 이때, React 는 이 컴포넌트를 위한 고유 상태 저장 공간 및 상태를 관리하는 `reducer` 함수를 내부적으로 연결
3. React 는 이 연결 정보를 기억하는 `dispatch` 함수 생성하여 반환

`dispatch` 함수는 자신이 생성될 때의 환경 (즉, 어떤 state 를 어떤 reducer 로 변경하여야 하는지에 대한 정보) 를 기억합니다. 이것이 클로저의 핵심 원리입니다.

즉, 우리가 `dispatch({ type: 'sidar' })` 를 호출하면, 이 디스패치 함수는 자신이 기억하고 있는 reducer 함수에게 해당 액션 객체를 정확하게 전달할 수 있게 됩니다. 

**한 컴포넌트 내부에 여러 `useReducer` 가 있더라도, 각 디스패치 함수는 자신만의 `state` 와 `reducer` 함수를 기억하는 별개의 클로저이기 때문에 서로 섞이지 않습니다.**

⇒ **이것이 왜 중요한가 : 함수의 동일성**

클로저를 사용하여 얻는 가장 큰 장점은 React 가 **디스패치 함수의 동일성을 보장**하기 때문입니다.

디스패치 함수는 컴포넌트가 리렌더링 되어도 절대로 새로 생성되지 않습니다. 첫 렌더링 시 생성된 함수가 그대로 유지됩니다.

- **이유** : 디스패치 함수 자체는 아무런 로직을 담고 있지 않고, 리액트 내부의 상태 관리 로직과 연결된 메신저로서 역할하기 때문입니다.
    
    실제 상태 값은 리액트가 별도로 관리하고, 디스패치는 그 상태를 변경해달라고 신호만 보내기 때문에 디스패치 함수는 자신이 기억하는 환경만 있으면 되므로, 매번 새롭게 생성될 이유가 없습니다.
    
- **장점** : `useEffect`, `useCallback`, `useMemo` 등의 의존성 배열에 디스패치 함수를 포함시켜도, 함수 자체는 변하지 않기 때문에 불필요한 실행이나 리렌더링을 유발하지 않습니다.
    
    ⇒ 즉, **React 성능 최적화에 매우 중요한 요소**로서 작용합니다.
    

```jsx
// 디스패치 함수는 변하지 않으므로, useEffect 는 맨 처음 한 번만 실행
useEffect(() => {
	...(중략)
	fetchData().then(data => {
		dispatch({ type : 'FETCH_SUCCESS', payload : data });
	});
}, [dispatch]); // 디스패치가 안정적이므로 안심 사용 가능
```