...
- **클라이언트 저장소 전략** 🍠
    ...
    - 📚 블로그 읽고 세션 하이재킹 정리해보기 🍠
        ...
        - 세션 하이재킹은 무엇인가요? 🍠
            ...
            **주요 공격 벡터**
            ...
            - XSS (자바스크립트로 `document.cookie` 읽음)
            - 네트워크 스니핑 (HTTP일 때)
    ...
    - 📚 블로그 읽고 CSRF 공격 정리해보기 🍠
        ...
        - 헤더 방식 인증의 주요 장점(예: CSRF 방어, 선택적 전송, CORS 단순화)을 정리해주세요. 🍠
            ...
            > 단, 토큰 저장·주입 로직을 직접 구현해야 하고 XSS에 대한 대비가 필수.
            > 
...
- [x]  이메일이 유효성 검사 조건을 충족했을 때만 '다음' 버튼이 활성화되어 다음 단계로 넘어갈 수 있도록 구현했나요?
...
- [x]  이메일 유효성 검사 충족 후 '다음' 버튼 클릭 시, 이전 이메일 정보가 상단에 표시된 채로 아래 이미지 처럼 **비밀번호 입력 컴포넌트**로 전환되도록 구현했나요?
...
- [x]  비밀번호 길이가 너무 짧을 경우(예: 6자 미만), 아래 이미지와 같이 "비밀번호는 6자 이상이어야 합니다."와 같은 **구체적인 에러 메시지**를 표시했나요?
...
- [x]  입력 필드 옆의 '눈 감은' 아이콘 버튼을 클릭하면, 입력된 비밀번호가 **보이거나 가려지도록(토글) 구현**하여 사용 편의성을 높였나요? (아래 이미지 참고)
...
- [x]  '비밀번호 재확인' 칸에 입력된 값이 첫 번째 비밀번호와 다를 경우, 아래 이미지와 같이 "비밀번호가 일치하지 않습니다."와 같은 **경고 메시지**를 표시했나요?
...
- [x]  비밀번호가 유효성 검사를 통과하고 서로 일치할 때만 아래 이미지와 같이 '다음' 버튼이 활성화되도록 구현했나요?
...
- [x]  사용자가 자신을 표현할 닉네임을 입력할 수 있는 필드를 구현했나요?
    
    **프로필 이미지 UI (선택)**
    
    - [x]  프로필 이미지 업로드 기능은 아직 배우지 않았으므로, UI만 간단하게 구현하여 시각적인 완성도를 높였나요? (실제 기능 구현은 필수가 아닙니다.)
...
- [x]  모든 정보를 입력하고 '회원가입 완료' 버튼을 클릭했을 때, 성공적으로 회원가입이 처리되도록 로직을 구현했나요?
- [x]  회원가입 이후 홈 화면으로 이동하나요?
...
- [x]  미션2에서 만든 로그인 기능을 포함하여 이번 회원가입 기능 전반에 **react-hook-form** 훅을 활용하여 폼 상태 및 유효성 검사를 관리했나요?
- [x]  **react-hook-form** 공식문서를 보며 학습한 내용을 정리해주세요.
    - **react-hook-form** 학습 내용 정리
        - `react-hook-form`은 리액트에서 **폼 상태를 간단하고 효율적으로 관리**하기 위한 라이브러리이다.
        - `useForm` 훅을 통해 입력값, 유효성 검사, 제출 로직을 쉽게 처리할 수 있다.
        - 불필요한 리렌더링을 줄여 **성능이 우수**하다.
        - `register`를 이용해 input을 연결하고, `handleSubmit`으로 제출 이벤트를 제어한다.
        - 다양한 **유효성 검사 옵션**과 함께 `yup`, `zod` 같은 스키마 검증 라이브러리와도 연동 가능하다.

**Zod 활용**

[Release notes | Zod](https://zod.dev/v4)

- [x]  **Zod** 라이브러리를 사용하여 이메일, 비밀번호, 닉네임 등의 **스키마를 정의**하고, 이를 **react-hook-form**과 연동하여 유효성 검사를 처리했나요?
- [x]  **Zod** 공식 문서를 보며 학습한 내용을 정리해주세요.
    - **Zod** 학습 내용 정리
        - **Zod**은 타입스크립트와 함께 쓰기 좋은 스키마 선언 및 유효성 검사 라이브러리이다.
        - `z.object`, `z.string()`, `z.number()` 등으로 스키마를 정의하고, `.parse()` 또는 `.safeParse()`로 값 유효성 검사를 수행할 수 있다.
        - 유효성 실패 시 `ZodError` 형태로 에러가 반환되며, 커스텀 에러 메시지 지정도 가능하다.
        - Zod v4부터는 성능 개선, 제너릭 단순화, `.meta()` 메타데이터, JSON Schema 변환 같은 기능이 추가되었다.
        - `.refine()`, `.transform()`, `.overwrite()` 등을 사용해 복잡한 검증이나 변환 로직도 스키마 안에서 선언 가능하다.

**타입 안정성 확보** 

- [x]  `TypeScript`를 사용하여 회원가입에 필요한 데이터(예: 이메일, 비밀번호, 닉네임)의 타입(interface 또는 type)을 명확하게 정의하고, 이를 활용하여 코드의 **타입 안정성**을 확보했나요?

**커스텀 훅 활용**

- [x]  `useLocalStorage`와 같은 **커스텀 훅**을 활용하여 사용자의 로그인 정보나 기타 필요한 데이터를 로컬 스토리지에 안전하게 저장하고 관리했나요? (예: 회원가입 성공 후 토큰 저장 등)
...
- 깃허브 주소
    ...
    https://github.com/aeongiing/9th_Web/tree/main/mission/chapter04/mission03
    
- 실행 영상
    
    [화면 기록 2025-10-12 오후 6.31.41.mov](attachment:195f6fbc-e382-471e-a102-66918b59f8aa:화면_기록_2025-10-12_오후_6.31.41.mov)
...
- 내가 리뷰한 내용을 캡처해 업로드
    ...
    ![image.png](attachment:38fcef85-ab09-46ed-969c-63bac84b6e05:image.png)
    
    ![image.png](attachment:da594634-2594-438b-9057-834d55110a6d:image.png)
    
- 받은 리뷰를 반영하여 개선한 코드와 캡처 업로드
    
    ![image.png](attachment:27ef96e8-3d02-4df7-98bc-facb9877adf5:image.png)
    
    ![image.png](attachment:46d295ed-0ec3-4aa2-b4ce-d0388df2999b:image.png)
    
    ![image.png](attachment:3b9a7fd6-8692-49ad-910c-6a464fc9e880:image.png)
    
    리뷰를 반영한 개선된 코드
    
    ![image.png](attachment:395d2911-cdcd-45e4-b06f-d52e6c47ca7c:image.png)
...
- 🍠 이슈 No.1
    - 이슈: 폼 유효성 오류인데 버튼이 활성화되는 문제가 생겼다.
    - 문제: errors/touched 상태와 빈 값 체크가 누락되어 발생한 문제임을 알 수 있었다.
    - 해결: isDisabled = hasErrors || hasEmpty 조건으로 비활성 처리하여 해결하였다.
...
- 📢 학습 회고 (예시, 서식만 복사하시고 지워주세요.)
    - 이해한 점: 인증과 인가의 차이와 정의, JWT-세션 차이에 대해 명확하게 알 수 있었다.
    - 어려운 점: 토큰 저장 위치와 XSS/CSRF 대응 정책 결정이 헷갈렸다.
    - 개선 방법: 보안 옵션(HTTPOnly/SameSite) 체크리스트화와 axios 인터셉터 표준화로 개선할 수 있음을 알게되었다.
    - 회고: 앞으로 공격에 대한 방어 전략으로써의 보안 정책에 대해 조금 더 구체적으로 알아보고 싶다는 생각이 들었다.
