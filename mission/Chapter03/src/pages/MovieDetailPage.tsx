...
const MovieDetailPage = (): ReactElement => {
    ...
    useEffect((): void => {
        const fetchMovie = async (): Promise<void> => {
            setIsPending(true);
            try {
                const data = await getMovie({ movieId: safeMovieId, language: "ko-KR" });
        ...
        const fetchCredit = async (): Promise<void> => {
            setIsPending(true);
            try {
                const data = await getCredits({ movieId: safeMovieId, language: "ko-KR" });
            ...
            <div className="max-w-5xl mx-auto p-6">
                <div className="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div className="flex-1">
                        ...
                        {tagline && (
                            <p className="mt-3 text-lg text-yellow-700 font-semibold">
                                {tagline}
                            </p>
                        )}
                ...
                <div className="mt-8">
                    ...
                    <div className="mt-6">
                        ...
                        <ul className="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {castTop.map((c) => (
                                <li key={`${c.credit_id}-${c.id}`} className="flex items-center gap-3 p-3 rounded-lg bg-white shadow-sm ring-1 ring-black/5">
                                    {c.profile_path ? (
                                        <img
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-gray-600">{c.character ?? "배역 정보 없음"}</p>
                                    </div>
                                </li>
                            ))}
                        </ul>
                ...
                <div className="mt-8">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        {detail.genres.map((g) => (
                            <span key={g.id} className="px-3 py-1 rounded-full bg-yellow-100 text-yellow-800 text-sm">
                                {g.name}
                            </span>
                        ))}
                    </div>
